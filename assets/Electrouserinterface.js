...
//      alert(i+","+j)
      ...
      $.getJSON('/electro_fetch_cart',function(data){
     //alert(data.data)
...
//       alert(key.includes(pid))
     ...
     $.getJSON('http://localhost:8000/fetch_all_electro_user_category',function(data){
       var htm=''
       //alert(JSON.stringify(data.data))

       data.data.map((item)=>{
           htm+=`<li><a href="http://localhost:8000/home1"  class="nav-link active" style="color: #fedcba;">${item.categoryname}</a></li>`
          // htm+=`<a href="#" class="bg-gray-900 text-white px-3 py-2 rounded-md text-sm font-medium" aria-current="page">${item.categoyname}</a>`
       })
        htm+=`<i class="material-icons" style="padding: 5px;color: white;">shopping_cart</i>`
       htm+=`<a href="/my_shopping_cart" style="text-decoration: none;"><span style="font-size: 13px;color: #fedcba;" id="shoppingcart" class="nav-link">(0 Articals)</span></a></span>`

       $('.mainmenu').html(htm)
     })


      $.getJSON('http://localhost:8000/fetch_all_electro_products',function(data){
       htm=''
       data.data.map(item=>{
         var price
         var offerprice
         ...
         htm+=`<a href='http://localhost:8000/electro_buy_product?product=${data}' style="text-decoration: none;color: black;" ><div style="display: flex; flex-direction: column; margin: 10px;align-items: center; width: 250px; height: 300px; padding: 5px; border: 1px solid #bdc3c7;box-shadow: 2px 2px #ecf0f1;elevation: below;border-radius: 10px;">
      ...
      <div style="width:200px;text-align:left;color: green;font-weight: bold;"><i>You save &#8377 ${save}</i></div>
       ...
       $('#productlist').html(htm)
         ...
         $.getJSON('http://localhost:8000/fetch_all_electro_user_subcategory',function(data){
       var htm=''
       //alert(JSON.stringify(data.data))
       data.data.map((item)=>{
...
//            $('#qtycomponent').show()
//            $('#qty').html(1)
//            $.getJSON('/electro_add_to_cart',{'product': $(this).attr('products')},function(data){
//         alert("add")
//})
//
//
...
//            alert("add")
     ...
     $.getJSON('/electro_add_to_cart',{'product': product,'qty': qty},function(data){
     //alert(data.data)
     var cart = JSON.parse(data.data)
     var key=Object.keys(cart)
     $('#shoppingcart').html(`(${key.length}) Articles `)
     ...
     function removeCart(productid){
        $.getJSON('/electro_remove_from_cart',{'productid': productid},function(data){
//                 alert("Removed")
                var cart = JSON.parse(data.data)
     var key=Object.keys(cart)
     $('#shoppingcart').html(`(${key.length}) Articles `)
